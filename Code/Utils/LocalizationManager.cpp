...
namespace Snake
{
...
#if !defined(_RELEASE)
		ConsoleRegistrationHelper::AddCommand("LocalizationDumpLoadedTags", LocalizationDumpLoadedTags, VF_NULL, "Dump out into about the loaded localization tags");
#endif
	}
	...
	void CGameLocalizationManager::LoadLocalizationData()
	{
		...
		//Log("[LocalizationManager] Loading localization file <%s>", locaFile.c_str());

		if (pLocMan && pLocMan->InitLocalizationData(locaFile.c_str()))
		{
			LoadTag(eLT_Init);
		}
		else
	...
	void CGameLocalizationManager::LegacyLoadLocalizationData()
	{
		...
		LogWarning("[LocalizationManager] Using Legacy localization loading");
		...
		{
			LogWarning("[LocalizationManager] Unable to find any Localization Data!");
		}
	}
	...
	void CGameLocalizationManager::LoadTagInternal(ELocalizationTag tag, const char* pTag)
	{
		...
		{
			LogWarning("[LocalizationManager] Failed to load localization tag <%s>", sTagLowercase.c_str());
		}
	}
	...
	void CGameLocalizationManager::UnloadTag(ELocalizationTag tag)
	{
		...
		if (!m_loadedTag[tag].empty())
		{
			...
			{
				LogWarning("[LocalizationManager] Failed to release localization tag <%s>", pTag);
			}
	...
	{
		CGameLocalizationManager* pMgr = g_pGame->pLocalization;
